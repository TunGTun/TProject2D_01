public static class SBossMinotaurStaticData
{
    public static readonly int MaxHP = 400;
    public static readonly float MoveSpeed = 3f;
    // public static readonly int Damage;
    //
    // public static readonly float StopDistance = 0.1f;
    //
    // public static readonly float LimitDistance = 10f;

    public static readonly float MoveToTargetMaxTime = 2f;

    //Skill One
    public static readonly float AttackOneTime = 1f;
    public static readonly float AttackOneRange = 3f;

    //Skill Two
    public static readonly float AttackFourTime = 1f;
    public static readonly float AttackFourRange = 3f;

    //Skill Three
    public static readonly float AttackThreeTime = 1f;

    //Skill Four
    public static readonly float TauntTime = 2f;
}
