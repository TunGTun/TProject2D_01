...
public class BossMinotaurSkillOne : IBossSkill
{
    ...
    public void Tick(BaseBossCtrl bossCtrl) // để update
    {
        ...
        switch (phase)
        {
            case EBossMinotaurState.MoveToTarget:
                ...
                if (bossPlayerDistance > SBossMinotaurStaticData.AttackOneRange) return;
                phase = EBossMinotaurState.AttackOne;
                bossCtrl.BaseBossState.ChangeState((bossCtrl.BaseBossState as BossMinotaurState).attackOne);
                break;

            case EBossMinotaurState.AttackOne:
